<template>
	<view class="order">
			<div class="tits"><text class="shu"></text>商品选择</div>
			<view  scroll-y  class="list">
			    <view class="null_s" v-if="!(data5&&data5.length)">
			      <image  class="null" src="../../static/images/img_null_bookbuilding.png" mode="widthFix"></image>
			      <view class="nText">暂无数据</view>
			    </view>
			    <view v-else class="list_item">
			      <!-- <view class="list_item" v-for="(project,i) in data5" :key="index"> -->
					<!--  <uni-collapse @change="changeCollapse(item)" v-model="index" :accordion="true">
					  	<uni-collapse-item 
					  					...
					  					<view class="top-left ">
					  						<radio-group style="margin-bottom: 10px;"  v-if="project.actions[0].checkType==1" @change="radioChange(data,idx)"  :value="project.id" v-for="(data,idx) in project.actions" :key="idx"> 
												<view class="" style="height: 17px;align-items: center;font-size: 20rpx;width: 100%;display: flex;">
													...
													<view class=" mr20 uni-ellipsis name">{{data.reprice}}</view>
												...
												<view class="" style="margin-left: 30px;font-size: 20rpx;width: 100%;display: flex;" v-if="data.children" v-for="(row, val) in data.children" :key="val+idx">
													...
													<view class=" mr20 uni-ellipsis name">{{row.reprice}}</view>
	...
	export default {
	    ...
	    methods: {
			...
			radioChange(item,index){
				...
				console.log(item,index,"uuuuuuuuuuu")
			},
			checkboxChange(item,index){
				console.log(item.id,"zzzzzzzzzzzz")  
					for (let i = 0; i < this.data5.length; i++) {
						for (let j = 0; j < this.data5[i].actions.length; j++) {
				 ...
				 console.log(this.data5,"this.packageHospitalListthis.packageHospitalList")           
			},
			// userChange(e){
					...
					// return
					 for (var i = 0;i< _this.data5.length;i++) {
						 ...
						 if((_this.current == _this.data5[i].actions[j].id)|| _this.data5[i].actions[j].checked){
							  var s = {
									 userId: uni.getStorageSync('userId'), //分配用户id
									 ...
									 price: _this.data5[i].actions[j].reprice, //: 服务、商品的价格只保存购买时的价格 ,
							...
							if (_this.data5[i].actions[j].children) {
								 ...
								 var s = {
								   ...
								   price:  _this.data5[i].actions[j].children[ji].reprice, //: 服务、商品的价格只保存购买时的价格 ,
					   ...
					   var shop = data;
					        var oldList = this.orderInfo.mshoppingCarts; // 首租订单购物车列表
					        var oldtian = []; // 首租带服务数据
					        for (var i =0;i<oldList.length;i++) {
					          if (oldList[i].billingType == 1) {
					            oldtian.push(oldList[i]);
					          }
					        }
					        var newtian = []; // 续租天数数据
					        var tian = []; // 续租天数数据number
					        ...
					        var oldArr = [];
					        var newArr = [];
					        for (var i = 0; i < newtian.length; i++) {
					          // console.log(newtian[i].commodityId)
					          newArr.push(newtian[i].commodityId);
					        }
					        for (var j = 0; j < oldtian.length; j++) {
					          // console.log(oldtian[j].commodityId)
					          oldArr.push(oldtian[j].commodityId);
					          if (newArr.indexOf(oldtian[j].commodityId) == -1) {
					            flag0 = false;
					            flag1 = false;
					          }
					        }
					        if (this.orderInfo.status == 4) {
					          if (newtian.length != oldtian.length) {
					            flag0 = false;
					          }
					        } else {
					          if (newtian.length < oldtian.length) {
					        ...
					        if (flag0) {
					          if (flag1) {
					            if (flag) {
					              ...
					              // return;
								  this.$ajaxs
								      .post('/console/v1/order/orderRenew?json='+encodeURI(shopjson)+'&status=1'+'&orderId='+this.orderInfo.id)
					                .then(res => {
					                  if (res.status == 200) {
					                ...
					                .catch(() => {});
									 ...
									 this.$showToast("请选择相同服务天数的设备！");
					              
					            }
					          } else {
								  this.$showToast("续租订单不可删减设备！");
					         
					          }
					        } else {
								this.$showToast("使用中的订单续租不可更换设备组合！");
